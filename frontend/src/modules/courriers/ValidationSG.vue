  ...
  <div>
    ...
    <div v-if="onglet === 'attente'">
      ...
      <div v-else-if="aValider.length === 0" class="msg-vide">
        Aucun courrier en attente de validation SG.
      </div>
      <div v-else>
  ...
  <div v-if="courrierExamine" class="modal-fond" @click.self="courrierExamine = null">
    <div class="modal" style="max-width:720px;max-height:90vh;display:flex;flex-direction:column">
      <div class="modal-titre">
        <i class="fa-solid fa-user-tie"></i> Validation SG — Proposition du SGA
      ...
      <div style="overflow-y:auto;padding:0 4px;flex:1">
        <div style="background:#f5f8ff;padding:12px;border-radius:6px;margin-bottom:16px;font-size:13px">
          <strong>{{ courrierExamine.numero_officiel }}</strong> — {{ courrierExamine.objet }}<br/>
          <span style="color:#666">{{ courrierExamine.expediteur }}</span>
        </div>

        <div style="background:#fff8e1;border:1px solid #ffe082;border-radius:6px;padding:10px;margin-bottom:16px;font-size:13px">
          <i class="fa-solid fa-lightbulb" style="color:#f9a825"></i>
          <strong> Proposition du SGA</strong> — Vous pouvez la valider telle quelle ou la modifier avant d'envoyer au DG.
        </div>
      ...
      <div class="actions-form">
        ...
        <button class="btn btn-success" @click="confirmerValidation" :disabled="enEnvoi">
          <i class="fa-solid fa-check"></i> Valider et envoyer au DG
        </button>
      </div>
    </div>
  </div>
...
const onglet = ref(props.ongletInitial)
...
const aValider = computed(() => props.courriers.filter(c => c.statut === 'EN_ATT_SG'))
...
async function chargerHistorique() {
  ...
  try {
    ...
    let data = rep.data.filter(c => c.statut !== 'EN_ATT_SG') // adapter pour SG
...
async function ouvrirFiche(c) {
  ...
  try {
    ...
    // Pré-remplir depuis proposition SGA
    const prop = c.proposition_sga || {}
    form.value = {
      destinataire_id: prop.destinataire_id || '',
      consignes_types: prop.consignes_types || [],
      consigne_libre:  prop.consigne_libre  || '',
    }
    // Pré-remplir les copies depuis la proposition SGA
  if (prop.copies_items && Array.isArray(prop.copies_items) && prop.copies_items.length > 0) {
  const newSelectionnees = []
  const newConsignes     = {}
  const newLibres        = {}

  prop.copies_items.forEach(i => {
    newSelectionnees.push(i.id)
    newConsignes[i.id] = [...(i.consignes_types || [])]  // copie indépendante
    newLibres[i.id]    = i.consigne_libre || ''
  })

  copiesSelectionnees.value = newSelectionnees
  copiesConsignes.value     = newConsignes    // remplacement complet → Vue détecte
  copiesConsigneLibre.value = newLibres
} else if (prop.copies_ids) {
  copiesSelectionnees.value = prop.copies_ids
} else {
  copiesSelectionnees.value = []
  copiesConsignes.value     = {}
  copiesConsigneLibre.value = {}
...
async function confirmerValidation() {
  ...
  enEnvoi.value = true
  try {
    await courriersApi.validerSg(courrierExamine.value.id, {
